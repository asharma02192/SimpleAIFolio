...
function TagsContent() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await apiFetch(`/api/tags/${editing.id}`, {
      ...
      cancel(); load(); toast(editing ? "Tag updated" : "Tag created", "success");
    ...
    <div className="min-h-screen flex" style={{ background: "var(--color-bg)" }}>
      ...
      <main className="flex-1 p-[var(--space-8)] max-w-[50rem]">
        <div className="flex items-center justify-between mb-[var(--space-8)]">
          ...
          {!creating && !editing && (
            <button onClick={startCreate} className="font-[family-name:var(--font-body)] text-[var(--text-sm)] font-500 px-5 py-2.5 transition-all duration-150 cursor-pointer hover:brightness-110" style={{ background: "var(--color-accent)", color: "var(--color-accent-on)", borderRadius: "var(--radius-md)", minHeight: "40px" }}>+ New Tag</button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-wrap gap-[var(--space-3)]">
            {tags.map((tag) => (
              <div key={tag.id} className="group flex items-center gap-[var(--space-2)] px-[var(--space-3)] py-[var(--space-1)]" style={{ background: "var(--color-bg-subtle)", borderRadius: "var(--radius-md)" }}>
                <span className="font-[family-name:var(--font-mono)] text-[var(--text-sm)]" style={{ color: "var(--color-text-secondary)" }}>#{tag.name}</span>
                <span className="font-[family-name:var(--font-mono)] text-[var(--text-xs)]" style={{ color: "var(--color-text-tertiary)" }}>{tag.postCount || 0}</span>
                <button onClick={() => startEdit(tag)} className="opacity-0 group-hover:opacity-100 font-[family-name:var(--font-mono)] text-[var(--text-xs)] cursor-pointer transition-colors hover:text-[var(--color-accent)]" style={{ color: "var(--color-text-tertiary)" }}>edit</button>
                <button onClick={() => handleDelete(tag.id)} className="opacity-0 group-hover:opacity-100 font-[family-name:var(--font-mono)] text-[var(--text-xs)] cursor-pointer transition-colors hover:text-[var(--color-error)]" style={{ color: "var(--color-text-tertiary)" }}>x</button>
              </div>
            ))}
          </div>
        )}
